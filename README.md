...
- `midnight-node` (`midnightntwrk/midnight-node:0.22.0`): Local Substrate consensus node on port `9944`, configured with `CFG_PRESET=dev` and `CHAIN=dev` (gated on block #1 authoring).
- `indexer` (`midnightntwrk/indexer-standalone:4.0.0`): Standalone GraphQL & WebSocket indexer on port `8088`, configured with `APP__APPLICATION__NETWORK_ID=undeployed`, `APP__INFRA__NODE__URL=ws://midnight-node:9944`, and quoted 64-character hex encryption key `APP__INFRA__SECRET` (probed via `/ready`).
